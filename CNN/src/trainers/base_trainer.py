...
class BaseTrainer:
    ...
    def __init__(self, config, log_dir):
        ...
        self.config = (
            config  # General config (model arch, optimizer, lr_scheduler, etc.)
        )
        self.trainer_config = self.config[
            "trainer_config"
        ...
        self.checkpoint_dir = ospj(self.trainer_config["save_dir"], self.config["name"])
        if not os.path.exists(self.checkpoint_dir):
            os.makedirs(self.checkpoint_dir)
        else:
            print(
                f"Warning! Save dir {self.checkpoint_dir} already exists!"
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if self.monitor_mode != "off":  # Then there is a metric to monitor
                if self.monitor_metric in log:  # Then we have measured it in this epoch
                    ...
                    save_condition = (
                        ...
                        and eval_result[self.monitor_metric.split("_")[-1]]
                        < self.monitor_best
                    ) or (
                    ...
                    if save_condition:
                        self.monitor_best = eval_result[
                            self.monitor_metric.split("_")[-1]
                        ]
                        path = os.path.join(self.checkpoint_dir, f"best_model.pth")
                        self.save_model(path=path)
                        self.logger.info(
                            f'Checkpoint saved, Step:{epoch}, Metric:{self.monitor_metric}, Best Value:{eval_result[self.monitor_metric.split("_")[-1]]}'
                        )
                    ...
                    if epoch == self.start_epoch:
                        self.prev_metric_val = eval_result[
                            self.monitor_metric.split("_")[-1]
                        ]
                        continue
                    ...
                    dropout_condition = (
                        self.monitor_mode == "min"
                        and eval_result[self.monitor_metric.split("_")[-1]]
                        >= self.prev_metric_val
                    ) or (
                        self.monitor_best == "max"
                        and eval_result[self.monitor_metric.split("_")[-1]]
                        <= self.prev_metric_val
                    )
                    if dropout_condition:
                        consecutive_degradation_steps += 1
                    else:
                        consecutive_degradation_steps = 0

                    if consecutive_degradation_steps >= self.early_stop:
                        break
                    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
                else:
                    ## The metric wasn't measured in this epoch. Don't change not_impoved_count or similar things here!!!
                    self.logger.warning(
                        f"Warning: At epoch {self.current_epoch} Metric '{self.monitor_metric}'"
            ...
            if self.current_epoch % self.save_period == 0:
                ...
                path = os.path.join(
                    self.checkpoint_dir, f"E{self.current_epoch}_model.pth"
                )
                self.save_model(path=path)
